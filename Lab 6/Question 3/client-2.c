...
int main()
{
        ...
        if (fork_status == 0) {
	        ...
	        int write_fd = open("messages-2.txt", O_WRONLY);
	        ...
	        int read_fd = open("messages-1.txt", O_RDONLY);

                do {
                        printf("%d", !waitpid(fork_status, &status, WNOHANG));
                        // *readbuf = NULL;
                        // scanf("%s", buf);
